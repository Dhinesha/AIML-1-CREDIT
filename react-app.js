...
function ReactFeaturesApp() {
  ...
  const filteredSkills = useMemo(() => {
    ...
    return baseSkills.filter((skill) => skill.toLowerCase().includes(keyword));
  }, [query]);
...
function ReactProjectsApp() {
  const projects = [
    ...
    {
      id: 3,
      title: "AgrogaurdAi (Farmer disease predict Agent)",
      category: "ai",
      summary:
    ...
    },
    {
      ...
      category: "data",
      summary: "Interactive Power BI dashboard for energy trend analysis.",
      stack: "Power BI",
    },
    {
      id: 5,
  ...
  return (
    <div>
      <div className="react-project-highlight">
        <h3 className="react-card-title">Featured: {featuredProject.title}</h3>
      ...
      <div className="react-project-toolbar">
        {categories.map((category) => (
          <button
            key={category}
            type="button"
            className={"react-chip" + (filter === category ? " active" : "")}
...
const reactProjectsRootElement = document.getElementById("react-projects-root");
